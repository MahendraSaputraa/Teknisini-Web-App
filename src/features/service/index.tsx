...
import { useCreateService } from "./hooks/use-create";
import { useUpdateService } from "./hooks/use-update";
import { DeleteModal } from "@/components/modal/delete-modal";
import { useDeleteService } from "./hooks/use-delete";

export default function ServiceTableData() {
  // hooks
  ...
  const { deleted } = useDeleteService({
    onSuccessCallback: () => {
      closeDeleteModal();
      setPayloadData(defaultValues);
      setErrors({});
  ...
  const { table, queryParams, setQueryParams, data, isPending } = useTableData({
    ...
    try {
      ...
      if (modalMode === "add") {
        await create.mutateAsync({
          ...
          duration_minutes: payloadData.duration_minutes,
  ...
  return (
    <div className="space-y-4">
      ...
      <TableData table={table} data={data} isPending={isPending} />
